...
TEST(LexerSuite, KeyWordTest){
  std::string_view input = "crEATE CREATE TABLE SELECT INSERT INTO WHERE VALUES DELETE FROM DROP";
  rdb::sql::Lexer lexer(input);
  std::string const tokens = dump_tokens(lexer);
  std::string const expected_tokens = 
      "Id 'crEATE' Loc=0:0\n"
...
TEST(LexerSuite, NumberTest){
  rdb::sql::Lexer lexer("+0 0.123456789 11 -11.121 +-. 0121 121. -plus 1.2.1");
